...
- **Response shaping:** none — return the full clipboard page as-is. Key fields of interest: `pyID`, `pyLabel`, `pyStatusWork`, `pxCurrentStageLabel`, `pxStageHistory[]`, `pxCoveredInsKeys[]`, plus whatever case-specific properties matter for the request (e.g. an `OutcomeList[]`-style field for a case type that has one).
